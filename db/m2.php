...
class dbstuff {
    ...
    function connect($dbhost='localhost', $dbuser, $dbpw, $dbname, $pconnect=FALSE, $force_db=FALSE, $new_link=FALSE) {
        ...
        if ($pconnect) {
            $this->link = @($GLOBALS["___mysqli_ston"] = mysqli_connect($dbhost,  $dbuser,  $dbpw));
        } else {
            $this->link = @($GLOBALS["___mysqli_ston"] = mysqli_connect($dbhost,  $dbuser,  $dbpw));
        }
    ...
    function find_database($tablepre) {
        $dbs = (($___mysqli_tmp = mysqli_query($this->link, "SHOW DATABASES")) ? $___mysqli_tmp : false);
        while($db = $this->fetch_array($dbs)) {
            ...
            while ($table = $this->fetch_array($q)) {
                if ($tablepre.'settings' == $table[0]) {
                    if (((bool)mysqli_query( $this->link, "USE $db['Database']"))) {
                        $this->db = $db['Database'];
                        return TRUE;
                    }
                }
    ...
    function field_name($query, $field) {
        set_error_handler($this->errcallb);
        $return = ((($___mysqli_tmp = mysqli_fetch_field_direct($query, 0)->name) && (!is_null($___mysqli_tmp))) ? $___mysqli_tmp : false);
        restore_error_handler();
        return $return;
    }
    ...
    function field_len($query, $field) {
        set_error_handler($this->errcallb);
        $return = ((($___mysqli_tmp = mysqli_fetch_fields($query)) && (isset($___mysqli_tmp[0]))) ? $___mysqli_tmp[0]->length : false);
        restore_error_handler();
        return $return;
    }
    ...
    function panic(&$sql) {
        ...
        if ($this->link === FALSE) {
            ...
            $error = ((is_object($this->link)) ? mysqli_error($this->link) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false));
            $errno = ((is_object($this->link)) ? mysqli_errno($this->link) : (($___mysqli_res = mysqli_connect_errno()) ? $___mysqli_res : false));
        }
    ...
    function query($sql, $panic = TRUE) {
        ...
        if (DEBUG) {
            if (LOG_MYSQL_ERRORS) {
                $this->last_id = ((is_null($___mysqli_res = mysqli_insert_id($this->link))) ? false : $___mysqli_res);
                ...
                if (mysqli_warning_count($this->link) > 0) {
                    ...
                    while ($row = mysqli_fetch_array($query3,  SQL_ASSOC)) {
	...
	$res = mysqli_data_seek($query, $row);
	$datarow = mysqli_fetch_arrray($res);
        restore_error_handler();
    ...
    function insert_id() {
        if (DEBUG and LOG_MYSQL_ERRORS) {
            ...
            set_error_handler($this->errcallb);
            $id = ((is_null($___mysqli_res = mysqli_insert_id($this->link))) ? false : $___mysqli_res);
            restore_error_handler();
        }
        return $id;
    }
